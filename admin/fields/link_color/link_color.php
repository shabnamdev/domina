 ...
 *
 */
if ( ! class_exists( 'DOM_Field_link_color' ) ) {
  class DOM_Field_link_color extends DOM_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'domina-pro' ),
        'hover'    => esc_html__( 'Hover', 'domina-pro' ),
        'active'   => esc_html__( 'Active', 'domina-pro' ),
        'visited'  => esc_html__( 'Visited', 'domina-pro' ),
        'focus'    => esc_html__( 'Focus', 'domina-pro' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="dom--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="dom-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
