 ...
 *
 */
if ( ! class_exists( 'DOM_Field_spacing' ) ) {
  class DOM_Field_spacing extends DOM_Fields {
    ...
    public function render() {
      ...
      $is_unit = ( ! empty( $unit ) ) ? ' dom--is-unit' : '';
      ...
      if ( ! empty( $args['all'] ) ) {
        ...
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[all]' ) ) .'" value="'. esc_attr( $value['all'] ) .'"'. $placeholder .' class="dom-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( $unit ) ? '<span class="dom--label dom--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
        ...
        foreach ( $properties as $property ) {
          ...
          echo '<input type="number" name="'. esc_attr( $this->field_name( '['. $property .']' ) ) .'" value="'. esc_attr( $value[$property] ) .'"'. $placeholder .' class="dom-input-number'. esc_attr( $is_unit ) .'" step="any" />';
          echo ( $unit ) ? '<span class="dom--label dom--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
          echo '</div>';
